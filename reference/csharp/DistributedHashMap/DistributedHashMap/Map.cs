...
namespace DistributedHashMap
{
    public class Map<T> : IMap<T>
    {
        ...
        public StateOptions DefaultStateOptions { get; set; } = new()
        {
            Concurrency = ConcurrencyMode.FirstWrite,
            Consistency = ConsistencyMode.Strong
        };

        private Header DefaultHeader => new()
        {
            MaxLoad = _maxLoad,
            Generation = (int) Math.Ceiling(Math.Log(Math.Max(256, _expectedCapacity)) / Math.Log(2)) - 7
        };

        protected string HeaderKey => "DHMHeader_" + Name;
        ...
        /// <returns></returns>
        public virtual Task Put(string key, T value, CancellationToken cancellationToken = default)
        {
            ...
            return DoRetry(async () =>
            {
                ...
                if (status.Item1 != null)
                    await _client.PublishEventAsync(status.Item1.PubSubName, status.Item1.Topic, status.Item1,
                        status.Item3 ?? new Dictionary<string, string>(),
        ...
        public async Task<T> Get(string key, CancellationToken cancellationToken = default)
        {
            ...
            var node = await GetRaw(bucketKey, cancellationToken);

            if (!node.Items.ContainsKey(key)) return default!;

            var serializedValue = node.Items[key];
            return JsonSerializer.Deserialize<T>(serializedValue, _client.JsonSerializerOptions)!;
        }

        private async Task<Node> GetRaw(string bucketKey, CancellationToken cancellationToken = default)
        {
            ...
            switch (saved)
            {
                case true when subscribe != null:
                    await _client.PublishEventAsync(subscribe.PubSubName, subscribe.Topic,
                            new TriggerEvent(key, Name, prevValue, null, subscribe.PubSubName, subscribe.Topic),
                            subscribe.Metadata ?? new Dictionary<string, string>(), cancellationToken)
                        .ConfigureAwait(false);
                    break;
                case false:
        ...
        public async Task Rebuild(CancellationToken cancellationToken = default)
        {
            ...
            if (!header.Rebuilding)
            {
                ...
                await _client.TrySaveStateAsync(_storeName, HeaderKey, header, _headerTuple.etag,
                    cancellationToken: cancellationToken).ConfigureAwait(false);
            }
            ...
            var nextGeneration = new Map<T>(Name, _storeName, _client)
            {
            ...
            header.Generation++;
            await _client
                .SaveStateAsync(_storeName, HeaderKey, _headerTuple.header, cancellationToken: cancellationToken)
                .ConfigureAwait(false);
        ...
        public async Task Unsubscribe(string key, CancellationToken cancellationToken = default)
        {
            ...
            bucket.etag = bucket.etag == string.Empty ? "-1" : bucket.etag;
            bucket.node ??= new Node();
            if (bucket.node.Triggers.ContainsKey(key)) bucket.node.Triggers.Remove(key);
        ...
        /// <returns></returns>
        protected static async Task<bool> DoRetry(Func<Task<bool>> task, CancellationToken cancel,
            int numberRetries = 100)
        {
            var taskStatus = await task().ConfigureAwait(false);
            if (cancel.IsCancellationRequested || numberRetries < 0) return false;
        ...
        private async Task<Header> GetHeaderAndMaybeRebuild(CancellationToken cancellationToken = default)
        {
            if (_rebuilding)
                return _headerTuple.header ??
                       throw new InvalidOperationException("Unable to locate header while rebuilding.");

            var header = await GetHeaderFromStore(cancellationToken).ConfigureAwait(false);
            if (!header.Rebuilding) return header;
        ...
        private int GetMapSize()
        {
            return (int) Math.Pow(2,
                7 + _headerTuple.header?.Generation ??
                throw new InvalidOperationException("Tried to calculate Map size without reading header first."));
        }
        ...
        {
            return GetBucketKey(GetBucket(key));
        }

        private string GetBucketKey(long bucketNum)
        {
            return
                $"DHM_{Name}_{_headerTuple.header?.Generation ?? throw new InvalidOperationException("Tried to get bucket key without reading header first.")}_{bucketNum}";
        }
        ...
        /// <param name="cancellationToken"></param>
        /// <returns></returns>
        private async Task<(TriggerEvent?, bool, Dictionary<string, string>?)> PutRaw(string key,
            string serializedValue, KeyTrigger? subscribe = null, CancellationToken cancellationToken = default)
        {
        ...
        public async IAsyncEnumerator<(string, T)> GetAsyncEnumerator(CancellationToken cancellationToken = new CancellationToken())
        {
            ...
            for (var i = 0; i < max; i++)
            {
                ...
                foreach (var (key, value) in node.Items)
                {
                    yield return (key, JsonSerializer.Deserialize<T>(value, _client.JsonSerializerOptions));
                }
            }
        }
    }
